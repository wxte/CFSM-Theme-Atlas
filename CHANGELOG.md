...
## v0.5.26 Mobile Usability

- 手机品牌左对齐，向下滚动收起品牌栏、向上滚动恢复，保留导航；加大按钮触摸区域。
- 节点采样条与网络曲线支持横向拖动查看，保留纵向页面滚动；取消锁定使用明确按钮。
- 图表坐标改为 HTML 文字，避免 SVG 拉伸；采样说明折叠，压缩概览卡片留白，长节点名最多显示两行。


## v0.5.25 Performance Pass

- 代码审查修复：延迟模块失败可重试，网络历史错误不再丢弃实时采样，保留同时间戳的增量字段；节点删除时释放小图观察器，非节点页面跳过隐藏表格定时更新；严格验证数值，移除失效 motion 初始化，恢复 WebSocket 15 秒连接超时（HTTP 轮询仍为 60 秒）。

- 修复 Globe 自动旋转缺失：通过本地 COBE 的 update API 按时间推进 phi，桌面最高 20 fps、轻量设备最高 12 fps；暂停后恢复不追赶后台时间，交互和减少动态效果设置均受尊重。保留无飞线、低 DPR / 采样和 idle 初始化。
