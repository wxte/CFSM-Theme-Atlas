export class DeferredNodeMap{
 ...
 load(){
  ...
  if(!this.promise)this.promise=import('./globe.js?v=0.5.22').then(({NodeMap})=>{const m=new NodeMap(this.onRegion);m.active=this._active;m.mode=this._mode;this.impl=m;if(this.lastUpdate)m.update(...this.lastUpdate);if(this.lastFocus)m.focus(this.lastFocus);return m;});
  return this.promise;
 }
