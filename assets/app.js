...
import {recordResources} from './resource-recorder.js?v=0.4.0';
...
import {windowSamples,historyFromArrays,aggregateHistory} from './network-core.js?v=0.4.0';
...
import {DeferredNodeMap} from './lazy-map.js?v=0.4.0';
...
function ensureNetworkCharts(){
 ...
 if(!networkChartsPromise)networkChartsPromise=import('./network.js?v=0.3.17').then(({NetworkCharts})=>{charts=new NetworkCharts();charts.live=chartState.live;charts.rangeMs=chartState.rangeMs;return charts;});
...
function renderNodeTrendsDeferred(row,s,history,enabled){
 ...
 import('./node-trends.js?v=0.3.17').then(mod=>{nodeTrendsModule=mod;if(row.querySelector('.node-detail')?.open)mod.renderNodeTrends(row,s,history,enabled);}).catch(()=>{});
}
...
const loadEnhancements=()=>import('./enhancements.js?v=0.3.12').catch(()=>{});
