import {HistoryAPI} from './history-api.js?v=0.5.1';
import {recordResources} from './resource-recorder.js?v=0.5.1';
import {highLoad,expiring,ActivityObserver} from './insights.js?v=0.5.1';
import {ViewRouter,pages,pageFromHash} from './router.js?v=0.5.1';
import {flag} from './flags.js?v=0.5.1';
import {windowSamples,historyFromArrays,aggregateHistory} from './network-core.js?v=0.5.1';
import {n,numeric,percent,fmtPct,ping,loss,bytes,online,uptime,month,trafficQuota,avg,total,costs,cycles,region,mergeSample} from './data.js?v=0.5.1';
import {DeferredNodeMap} from './lazy-map.js?v=0.5.1';
import {Plot} from './plot.js?v=0.5.1';
...
function ensureNetworkCharts(){
 ...
 if(!networkChartsPromise)networkChartsPromise=import('./network.js?v=0.5.1').then(({NetworkCharts})=>{charts=new NetworkCharts();charts.live=chartState.live;charts.rangeMs=chartState.rangeMs;return charts;});
...
function renderNodeTrendsDeferred(row,s,history,enabled){
 ...
 import('./node-trends.js?v=0.5.1').then(mod=>{nodeTrendsModule=mod;if(row.querySelector('.node-detail')?.open)mod.renderNodeTrends(row,s,history,enabled);}).catch(()=>{});
}
...
const loadEnhancements=()=>import('./enhancements.js?v=0.5.1').catch(()=>{});
