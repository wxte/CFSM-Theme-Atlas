...
WXT Atlas 以 Parallel Routes Demo 的页面骨架、Catalyst 的组件层级和 HomeDash 的监控信息密度为主要设计参考。主题保持原生 HTML / CSS / JavaScript，不引入 Vue、React、ECharts 等运行时框架。
...
实时模式使用当前页面收到的约 5 分钟采样窗口，约每 5 秒形成页面快照。
