...
主题只使用根目录 `index.html` 与 `assets/*`，不需要构建或修改后端。COBE 2.0.1、地图都已放入 `assets/`，无需为本主题添加外部 CDN 的 CSP 白名单。
...
- 地球默认静止，拖动或方向键旋转；尺寸、主题、节点位置/状态变化时按需绘制。无持续地球动画循环。无 WebGL 时使用平面地图。
- Lite 默认开启，关闭非必要 CSS 动效；退出 Lite 后在节点 hover 时显示脉冲、平面连线 hover 时流动。系统减少动态效果偏好仍优先。
