...
- 联通 / 电信 / 移动延迟与丢包历史
- 1H / 6H / 24H / 7D 网络历史
- COBE 点阵地球、地区标记与地区筛选联动（不绘制飞线）
- 桌面节点使用紧凑长条列表；手机端使用自适应节点卡片
- 手机端网络图切换；地球自动使用更低 DPR、采样与标签上限
- 节点详情原位展开
- 服务器离线 / 恢复上线活动记录
- 浅色 / 深色 / 跟随系统
...
地球只保留节点标记、地区标签与手动旋转，不再绘制飞线，也不再后台自动旋转。桌面端降低最高 DPR / 点采样，拖动时暂停标签布局；手机和 Save-Data / 低性能环境进一步降低采样与标签数量，避免 WebGL 抢占滚动与首屏资源。
...
- 桌面 motion CSS / JS 在 load 后空闲加载；手机端不加载 motion JS，只保留轻量密度辅助逻辑
- 本地 JetBrains Mono 字体资源；中文使用系统中文字体
- 非关键 command palette / toast 延迟加载
- 地球本身在 KPI / 节点首屏完成后再初始化
- 地球离屏时停止不必要渲染；飞线关闭，降低 DPR / 点采样
- 地球按时间推进低速自转，桌面最高 20 fps、轻量设备最高 12 fps；离屏、后台、关闭、拖动及标签交互时暂停，尊重减少动态效果设置；拖动期间暂停标签 DOM 布局
- 手机 / Save-Data / 低性能环境降低 WebGL DPR、地图采样与标签数量
- 网络历史按时间范围下采样
